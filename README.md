# QPT Framework - Quvia Performance Toolkit

[![Python 3.9+](https://img.shields.io/badge/python-3.9+-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

**QPT (Quvia Performance Toolkit)** is a unified performance testing framework that seamlessly integrates Playwright, k6, JMeter, and custom workflows into a single, powerful testing platform with real-time monitoring and distributed agent support.
...
- **⏱️ Temporal Workflows**: Orchestrate complex, long-running performance tests with built-in retry and state management
...
- **🤖 CLI Tool**: `qpt` for agent management and deployment
...
```bash
# Clone repository
git clone https://bitbucket.org/espacenetworks/qpt.git
cd neuron-perf-test
...
# Make CLI executable
chmod +x qptcli.py
sudo ln -s $(pwd)/qptcli.py /usr/local/bin/qpt
```
...
# Run async distributed browser test 
pytest examples/09_async_distributed_browsers.yml

# Run weighted load distribution 
...
### Advanced
- **[Temporal Workflows](docs/TEMPORAL_TESTING.md)** - Orchestrate long-running tests with Temporal 
...
### 4. Async Agents with Browser Contexts 
```yaml
agents:
  browser-agent:
...
temporal:
  workflows:
    long_running_perf_test:
      workflow_id: "perf-test-2024-01"
...
### CLI Tool
- **qpt**: Command-line interface for agent management
...
qpt agent deploy --name prod-monitor --target admin@prod.com --type docker
...
```typescript
const agent = new QPTBrowserAgent({
  name: 'my-app',
  mode: 'emit',
